...
- 정책은 회전·그림자처럼 카드 원본 사각형 밖으로 나가는 요소를 `visualOverflow`로 받을 수 있다. Home은 32dp의 시각 여유를 전달해 그림자가 화면 경계에서 잘릴 가능성을 줄인다.
- 카드 회전은 재구성마다 바뀌지 않지만, 화면을 새로 만들면 ±7° 범위에서 다시 무작위로 정한다. 저장된 사용자 설정처럼 영속할 값이 아니므로 Domain 모델에 넣지 않는다.
