...
- 관련 파일: [`feature/onboarding/select/OnboardingPhotoSelectViewModel.kt`](../../app/src/main/java/com/example/sairo14/feature/onboarding/select/OnboardingPhotoSelectViewModel.kt)
- 관련 파일: [`feature/onboarding/select/OnboardingPhotoSelectScreen.kt`](../../app/src/main/java/com/example/sairo14/feature/onboarding/select/OnboardingPhotoSelectScreen.kt)
...
사진 선택·분석·결과 Route가 백스택에 있는 동안 해당 화면 ViewModel은 유지된다. 선택 화면이 뒤로가기로 다시 표시될 때는 같은 `NavEntry`이므로 선택 상태와 Pager에서 확인한 `viewedPhotoIds`가 남는다. `HorizontalPager`는 현재 페이지가 바뀔 때 `markPhotoViewed()`를 호출하고, 진행 표시 줄은 `photos.size`만큼 그리며 선택 사진을 초록색, 확인했지만 선택하지 않은 사진을 진한 회색, 아직 확인하지 않은 사진을 회색으로 구분한다. 새 탐색은 이전 선택 Route를 제거하고 다른 `searchSessionId`를 가진 Route를 추가하므로 새 ViewModel이 생성된다.
...
- 확인 상태는 서버에 저장하는 데이터가 아니라 현재 사진 선택 화면의 탐색 피드백이다. 따라서 ViewModel 수명 동안만 유지하고 새 탐색에서는 초기화한다.
