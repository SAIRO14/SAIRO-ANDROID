...
3. 로딩 ViewModel은 새 분석 또는 재시도 전에 기존 `Job`을 취소하고 화면 상태용 세대 번호를 올린다. 늦게 도착한 이전 성공·실패 응답은 현재 세대와 다르므로 UI에 반영하지 않는다.
4. UseCase는 API 호출 전에 세션 저장소에서 `OnboardingAnalysisRequestToken`을 발급받는다. 저장소는 Mutex 안에서 세션별 토큰을 증가시키고 최신 요청으로 등록하므로, Loading ViewModel이 다시 생성돼도 이전 요청과 같은 토큰을 쓰지 않는다. 성공 응답은 최신 토큰과 일치할 때만 저장하며 실패 결과는 저장하지 않는다.
...
- 결과 화면은 DataStore가 발급한 완료 상태 토큰을 먼저 등록하고, 같은 토큰이 최신일 때만 추천 유무에 따른 완료 상태를 저장한다. 이전 세션의 늦은 저장은 최신 토큰과 달라 무시된다. 최신 상태 저장 자체가 실패하면 결과 화면은 오류를 표시하며, 사용자는 재시도로 새 토큰과 함께 상태 저장을 다시 요청한다.
...
- 취소는 Retrofit 같은 협조적 suspend 작업을 즉시 중단한다. 외부 구현이 취소를 따르지 않아도 ViewModel의 요청 세대 비교와 세션 저장소의 토큰 발급·원자 저장이 오래된 응답의 UI·세션 반영을 막는다.
