...
어댑터는 이를 px로 변환하고 실제 MapView 크기를 넘지 않도록 보정한 뒤 카카오 지도에 적용한다.
따라서 시트가 완전히 확장돼 헤더·일차 선택 영역과 시트의 여백이 겹치더라도 SDK에 유효하지 않은
padding을 전달하지 않는다. 선택한 장소의 지리 좌표는 가려진 전체 MapView가 아니라 실제 보이는 지도
영역의 중심에 배치된다.
...
- 시트와 상단 UI가 겹칠 때 요청한 top·bottom padding 합계가 지도 높이를 넘을 수 있다. 어댑터는
  실제 MapView 크기 안에서 top·left를 우선 유지하고 bottom·right를 제한한다.
