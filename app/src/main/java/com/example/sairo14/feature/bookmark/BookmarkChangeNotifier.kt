...
/** 화면 간에 성공한 북마크 변경을 순서대로 전달하는 메모리 기반 통지자다.
 *
...
class BookmarkChangeNotifier @Inject constructor() {
    ...
    /** 현재 화면이 수신할 수 있는 북마크 변경 흐름이다. */
    val changes: SharedFlow<BookmarkChange> = _changes.asSharedFlow()

    /** 저장 또는 해제 성공 결과를 이전 화면에 알린다.
     *
     * 활성 수집자의 버퍼가 가득 차면 새 변경을 버리지 않고 발행 coroutine을 일시 중단한다.
     * @param change 서버 성공 뒤 화면 간에 동기화할 북마크 상태
     */
    suspend fun notify(change: BookmarkChange) = _changes.emit(change)
}
