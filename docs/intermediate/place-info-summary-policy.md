...
상세 화면의 태그는 운영시간, 휴무일, 주차, 문의처 순서로 만든다. 절기·월별 운영시간과 평일·주말 운영시간은 각각 독립된 태그로 펼쳐 표시한다. 동일한 `PhoneInquiry`는 UI 모델의 `distinct()` 처리로 한 번만 표시한다.
...
운영시간·휴무일·주차 정보가 모두 없고 문의처 전화번호만 표시되는 장소에는 전화번호 태그 앞에 `PhoneInquiry`를 추가한다. 다른 장소 정보가 있으면 문의처는 전화번호만 표시한다.
